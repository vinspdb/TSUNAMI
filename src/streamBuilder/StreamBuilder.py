    ...
    def __generateStream(self, start: str, end: str):
        ...
        with alive_bar((lastDay-currentDay).days, force_tty=True) as bar:
            while currentDay != lastDay:
                ...
                self.__window.clean()
                check_day = self.__insertLabeledExamples(examplesOfDay, currentDay)
                # print(check_day)
                if check_day == True:
            ...
            with open('brazilian_list_time_gen.pickle', 'wb') as handle:
                pickle.dump(list_time, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
try:
    ...
    StreamBuilder('localhost', 'root', '@lienG@n2022', 'prova_db', 4, 4, 2,
                  )
